...
FaceID/matcher.py
=================
Matches a detected face embedding against the offender database.
Uses cosine similarity. Returns match + person details if found.
"""
...
def match_face(embedding: np.ndarray) -> Optional[dict]:
    """
    Compare embedding against all registered offenders.

    Returns
    -------
    dict with keys: id, name, email, phone, uid_ref, similarity
...
def get_embedding(app, frame) -> Optional[tuple]:
    """
    Detect face in frame and return (embedding, bbox, face_crop).
    ...
    # Crop face for saving as evidence
